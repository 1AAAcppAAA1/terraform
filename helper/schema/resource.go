...
type Resource struct {
	...
	// schema version to the current schema. It is specifically by Terraform
	// when the stored schema version is less than the current SchemaVersion of
	// the Resource.
	//
	// This StateUpgradeFunc takes the schema version, the decoded state json,
	// using the default types in a map[string[interface{}, and the provider
	// meta value. The returned map value should encode into the proper format
	// json to match the current provider schema.
	//
	// UpgradeState is the replacement for MigrateState.
